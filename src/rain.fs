...
uniform sampler2D texture0;    // La texture rendue (équivalent à iChannel0)
uniform float iTime;           // Temps
...
void main()
{
    // Convertir les coordonnées pour correspondre à Shadertoy
    ...
    // Utiliser textureLod avec raylib (texture0 au lieu de iChannel0)
    vec3 col = textureLod(texture0, UV+n, focus/10.0).rgb;
